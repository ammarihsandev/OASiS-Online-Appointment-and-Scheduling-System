...
$Lecturer_ID = $_SESSION['myuser'];
		$query="SELECT * FROM message WHERE Lecturer_ID = '".$Lecturer_ID."' AND Sender = 'lecturer' ORDER BY Message_ID DESC ";
...
$dateo = substr($row['Message_time'], 0, 10); 
 $username_array[] = "<tr><td><center>".$status."</center></td><td>".substr($name, 0, 20)."</td><td>".substr($row['Message_content'], 0, 50)."</td><td><center>".$dateo."</center></td><td><center>
	<a href=\"viewsentmsg.php?Message_ID=".$row['Message_ID']."&Student_ID=".$row['Student_ID']."\"><img title=\"View Message\" width=\"24\" height=\"24\"  border=\"0\" src=\"../image/sentmsg.png\"/></a>	
	</center></td></tr>";
}
}
$username_string = implode("", $username_array); 
	
$content="<div  align=\"right\"><a href=\"message.php\"><img  border=\"0\" src=\"../image/inboxbtn.png\" title=\"View Inbox Message\"></img></a></div>
